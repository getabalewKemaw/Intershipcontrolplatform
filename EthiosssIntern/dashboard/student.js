        ...
        document.querySelectorAll('.btn-apply').forEach(button => {
            button.addEventListener('click', (e) => {
                if (!button.classList.contains('applied')) {
                    applyForInternship(e.target.closest('.btn-apply').dataset.id, studentId);
                }
            });
        });
    }, 1000);
...
function applyForInternship(internshipId, studentId) {
    let internships = JSON.parse(localStorage.getItem('internships')) || [];
    ...
    // Update UI
    const button = document.querySelector(`.btn-apply[data-id="${internshipId}"]`);
    if (button) {
